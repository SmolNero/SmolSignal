...
export function buildIrFile(remoteName: string, buttons: IrButton[]): IrBuildResult {
  ...
  normalizedButtons.forEach(({ normalized }) => {
    const key = normalized.name.toLowerCase();
    if (seenNames.has(key)) duplicateNames.add(normalized.name);
    seenNames.add(key);
  });

  duplicateNames.forEach((name) => errors.push(`${name}: button names must be unique.`));
  ...
  const content = [`Filetype: IR signals file`, `Version: 1`, `# Generated by SmolSignal safe IR builder`, body, ""].join("\n");
...
export function parseIrButtonsFromCapture(content: string) {
  const lines = content.replace(/^\uFEFF/, "").replace(/\r\n/g, "\n").split("\n");
  const buttons: IrButton[] = [];
  const errors: string[] = [];
  let current: Partial<IrButton> = {};

  function pushCurrent() {
    if (!current.name && !current.protocol && !current.address && !current.command) return;
    if (current.name && current.protocol && current.address && current.command) {
      buttons.push({
        name: current.name,
      ...
      errors.push(`${current.name || "Unnamed button"}: missing protocol, address, or command.`);
    }
    current = {};
  }
  ...
  lines.forEach((line) => {
    ...
    if (key === "address") current.address = value;
